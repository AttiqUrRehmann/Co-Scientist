...
def test_backoff_grows_exponentially() -> None:
    policy = RetryPolicy(max_attempts=5, base_ms=1_000, cap_ms=60_000)
    delays = [
...
def test_backoff_is_capped() -> None:
    policy = RetryPolicy(max_attempts=20, base_ms=1_000, cap_ms=5_000)
...
def test_rate_limit_backoff_may_exceed_the_ordinary_cap() -> None:
    policy = RetryPolicy(max_attempts=8, base_ms=1_000, cap_ms=60_000)
    delay = backoff_seconds(6, "rate limit", policy, jitter=1.0)
    assert delay > 60.0


def test_jitter_keeps_delays_inside_half_to_one_and_a_half() -> None:
    policy = RetryPolicy(max_attempts=5, base_ms=2_000, cap_ms=60_000)
    for _ in range(50):
