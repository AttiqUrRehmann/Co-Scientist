...
def _embeddings_row(cfg) -> str:
    from .config import has_embeddings_key

    if has_embeddings_key(cfg):
        return cfg.embeddings.model
    return (
        "[yellow]hash fallback — set VOYAGE_API_KEY or OPENAI_API_KEY "
        "for real dedup[/yellow]"
    )


@app.command()
def doctor(ctx: typer.Context) -> None:
    """Check that the configured backend, its credentials, and tools all work.

    Costs nothing: it resolves keys, probes CLI versions, and runs the MCP
    ...
    if is_cli:
        # Only meaningful for a backend that shells out to something.
        tbl.add_row(
    ...
    # provenance. An API provider gets both from the API itself and never
    # launches it, so a failure there must not fail its preflight.
    ok_mcp = True
    if is_cli:
        ok_mcp, mcp_detail = _check_mcp()
        tbl.add_row(
            "MCP server", "[green]ok[/green]" if ok_mcp else f"[red]{mcp_detail}[/red]"
        )
        if ok_mcp:
...
def init(ctx: typer.Context) -> None:
    ...
    tbl.add_row(
        "backend",
        f"{status.backend} ({status.binary})" if is_cli else status.backend,
    )
    if is_cli:
